...
class SnuddaSimulate(object):
    ...
    def __init__(self,
                 ...
                 distributePolicy=None):

        self.verbose = verbose
        self.log_file = log_file
        self.distributePolicy = distributePolicy
    ...
    def distribute_neurons(self):
        ...
        policy = self.distributePolicy
        if policy == None:
            policy = "RoundRobin"
        
        self.write_log("Distributing neurons with policy " + policy)
        ...
        elif policy == "ContinuousSegment":
            nhost = int(self.pc.nhost())
            id = int(self.pc.id())
            segSize = int(self.num_neurons // nhost)
            segTail = self.num_neurons - nhost * segSize
            cnt = 0
            self.neuronNodes = []
            for segID in range(0, nhost):
                detailedSegSize = segSize
                if segID < segTail:
                    detailedSegSize += 1
                segBegin = cnt
                segEnd = cnt + detailedSegSize
                if segID == id:
                    self.neuron_id = range(segBegin, segEnd)
                for j in range(segBegin, segEnd):
                    self.neuron_nodes.append(segID)
                cnt += detailedSegSize
            print("worker " + str(id) + " has neuronID " + str(self.neuron_id) + ", neuronNodes " + str(self.neuron_nodes))
            assert cnt == self.num_neurons, "distributed node counter mismatch"
        else:
            assert false, "distributePolicy undefined"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--distributePolicy","--distributePolicy",
                      default="RoundRobin",
                      help="Distribution policy for neuron network partitioning")
    ...
    rank = sim.pc.id()
    print("id:", sim.pc.id())
    print("nhost:", sim.pc.nhost())
    sim.pc.barrier()
    if rank == 0 and coredatPath != None and not os.path.isdir(coredatPath):
        os.mkdir(coredatPath)
    sim.pc.barrier()
    h.secondorder = 1
